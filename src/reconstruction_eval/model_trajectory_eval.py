...
def calculate_eval_metrics(model:ReconsModel, eval_data:Dict, train_data:Dict,
    ...
    traj_metrics["chamfer_normalization"] = {
        ...
        "training_scene_diameter": round(float(training_diameter), 4),
        "training_aabbox_diagonal": round(float(training_bbox_diameter), 4),
        "full_scene_diameter": round(float(full_diameter), 4),
        "full_aabbox_diagonal": round(float(full_bbox_diameter), 4),
